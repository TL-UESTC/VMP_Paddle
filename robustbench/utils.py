...
def export_weight_names(net):
    ...
    with open('ckpt\cifar100\corruptions\Hendrycks2020AugMix_ResNeXt_paddle.txt', 'w') as f:
...
def load_model(model_name: str,
               ...
               norm: Optional[str] = None) -> nn.Layer:
    ...
    # print('threat_model_',threat_model_)
    ...
    if not isinstance(models[model_name]['gdrive_id'], list):
        ...
        paddle_list = open('ckpt\cifar100\corruptions\Hendrycks2020AugMix_ResNeXt_paddle.txt')
        checkpoint = torch.load(model_path, map_location=torch.device('cpu'))
        
        # print('22222222222')
        # checkpoint = torch.load(model_path, map_location=paddle.device.set_device('cpu'))
        ...
        for p in paddle_list:
            ...
            if t in torch_list:
                ...
                print(p)
        f = open('ckpt\cifar100\corruptions\Hendrycks2020AugMix_ResNeXt_paddle.pdparams', 'wb')
        import pickle
        pickle.dump(paddle_state_dict, f)
        f.close()
        model_state_dict = paddle.load('ckpt\cifar100\corruptions\Hendrycks2020AugMix_ResNeXt_paddle.pdparams')
        model.set_state_dict(model_state_dict)
        # paddle.save(model.state_dict(), "myself.pdparams")
        # print(paddle.load('myself.pdparams'))
        # assert(False)
...
# # '''
    ...
    # If we have an ensemble of models (e.g., Chen2020Adversarial)
        ...
        for i, gid in enumerate(models[model_name]['gdrive_id']):
            ...
            #                         map_location=paddle.device.set_device('cpu'))
            # try:
...
def _safe_load_state_dict(model: nn.Layer, model_name: str,
                          state_dict: Dict[str, paddle.to_tensor],
                          dataset_: BenchmarkDataset) -> nn.Layer:
...
def clean_accuracy(model: nn.Layer,
                   x: paddle.to_tensor,
                   y: paddle.to_tensor,
                   batch_size: int = 100,
                   device: paddle.device.get_device = None):
    if device is None:
        # device = x.device
        x_device = paddle.device.get_device()
    acc = 0.
    ...
    with paddle.no_grad():
        for counter in range(n_batches):
            ...
            # paddle.device.set_device(x_device)
            ...
            # print(y_curr)
            # print(output.argmax(1) == y_curr)
            # print('output.shape', output.shape)
            # print('output.max(1)[1]', output.max(1)[1])
            # print('y_curr', y_curr)
            # print('output.argmax(1)', output.argmax(1))
            # print('11111111111')
            acc += paddle.to_tensor(output.argmax(1) == y_curr, dtype='float32').sum()
            count_x += x_curr.shape[0]
            print('{:.2f}'.format((acc / count_x).item()), end=' ')
