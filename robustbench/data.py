...
def _load_dataset(
        dataset: Dataset,
        n_examples: Optional[int] = None) -> Tuple[paddle.to_tensor, paddle.to_tensor]:
    batch_size = 100
    ...
    x_test_tensor = paddle.concat(x_test,axis=0)
    y_test_tensor = paddle.concat(y_test,axis=0)
...
def load_cifar10(
        ...
        data_dir: str = './data',
        prepr: Optional[str] = 'none') -> Tuple[paddle.to_tensor, paddle.to_tensor]:
...
def load_imagenet(
        ...
        data_dir: str = './data',
        prepr: str = 'Res256Crop224') -> Tuple[paddle.to_tensor, paddle.to_tensor]:
    transforms_test = PREPROCESSINGS[prepr]
...
CleanDatasetLoader = Callable[[Optional[int], str], Tuple[paddle.to_tensor,
                                                          paddle.to_tensor]]
...
def load_clean_dataset(dataset: BenchmarkDataset, n_examples: Optional[int],
                       data_dir: str, prepr: Optional[str] = 'none') -> Tuple[paddle.to_tensor, paddle.to_tensor]:
...
def load_corruptions_cifar(
        ...
        data_dir: str,
        corruptions: Sequence[str] = CORRUPTIONS,
        shuffle: bool = False) -> Tuple[paddle.to_tensor, paddle.to_tensor]:
    assert 1 <= severity <= 5
    n_total_cifar = 10000
    ...
    x_test = paddle.to_tensor(x_test)[:n_examples]
    y_test = paddle.to_tensor(y_test)[:n_examples]
