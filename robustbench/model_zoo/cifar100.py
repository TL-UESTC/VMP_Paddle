...
class Chen2020EfficientNet(WideResNet):
    def __init__(self, depth=34, widen_factor=10):
        ...
        self.register_buffer(
            'mu',
            paddle.to_tensor([0.5071, 0.4867, 0.4408]).reshape([1, 3, 1, 1]))
        self.register_buffer(
            'sigma',
            paddle.to_tensor([0.2675, 0.2565, 0.2761]).reshape([1, 3, 1, 1]))
        ...
        self.register_buffer(
            ...
            paddle.to_tensor(
                [0.5070751592371323, 0.48654887331495095,
                 0.4409178433670343]).reshape([1, 3, 1, 1]))
        self.register_buffer(
            'sigma',
            paddle.to_tensor(
                [0.2673342858792401, 0.2564384629170883,
                 0.27615047132568404]).reshape([1, 3, 1, 1]))
...
class Rice2020OverfittingNet(PreActResNet):
    def __init__(self):
        ...
        self.register_buffer(
            'mu',
            paddle.to_tensor(
                [0.5070751592371323, 0.48654887331495095, 0.4409178433670343]).reshape([1, 3, 1, 1]))
        self.register_buffer(
            'sigma',
            paddle.to_tensor(
                [0.2673342858792401, 0.2564384629170883,
...
class Hendrycks2020AugMixResNeXtNet(CifarResNeXt):
    def __init__(self, depth=29, cardinality=4, base_width=32):
        ...
        self.register_buffer('mu', paddle.to_tensor([0.5] * 3).reshape([1, 3, 1, 1]))
        self.register_buffer('sigma', paddle.to_tensor([0.5] * 3).reshape([1, 3, 1, 1]))
